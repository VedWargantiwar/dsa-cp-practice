# DSA & Competitive Programming Journey

This repository contains my C++ solutions and practice code for Data Structures, Algorithms, and Competitive Programming. 

## Structure
* `learning_cpp/` - Core C++ concepts (Pointers, Structs, Memory)
* `functions_cpp/` - Custom functions and pass-by-reference logic
* `array_questions_cpp/` - Basic array manipulation
* `loop_questions_cpp/` - Iteration and logic building
* `Hacker_Rank_cpp/` - Solutions to HackerRank C++ domain problems
* `leetCode/` - Solutions to LeetCode C++ domain problems
* `CodeChef/` - Solutions to CodeChef C++ domain problems

*(folder for Codeforces will be added as the journey continues).*